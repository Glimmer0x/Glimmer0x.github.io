...
const timeline: TimelineEntry[] = [
  {
    ...
    highlights: [
      "Built conversational task execution system supporting text & voice interaction for **1.6M DAU** — enabling fund transfers, batch purchases, and market analysis via natural language",
      "Designed multi-stage execution framework: session management → intent resolution → task planning → tool orchestration → verification → response generation",
      "Reduced end-to-end latency via structured generation with **SGLang** for direct DAG emission, using fine-tuned GPT and LoRA-adapted open-source models",
      "Spearheaded fuzzy search initiative, increasing search hit rates by **42.1%** and user retention by **1.7%** for **20M+ users**",
      "Developed Two-Stage Ranking fuzzy search algorithm, reducing time complexity from **O(N²) to O(N log N)** and space from **O(N²) to O(N)**",
